...
{
	if (request->dev_addr != dev_addr && request->dev_addr != 0)
		return false;

	if (crc16((uint8_t*) request, sizeof(struct modbus_request)) != 0)
		return false;

	response->dev_addr = request->dev_addr;
	...
	if (request->func == 3)
	{
		...
		if (response->data_len + 5 > UINT8_MAX)
			return false;
		...
		{
			response->data[i * 2] = 0;
			response->data[i * 2 + 1] = (*((volatile uint8_t*) (reg_addr + i)));
		}

		uint16_t crc = crc16((uint8_t*) response, response->data_len + 3);
		response->data[i * 2] = crc;
		response->data[i * 2 + 1] = crc >> 8;
